...
const products = [
  {
    title: "금속 각종 시험편",
    detail: "인장시험편 · 충격(샤르피) KS3·4·5호 · 압축 · 굽힘",
  ...
  {
    title: "각종 감속기 교체용 장비",
    detail: "로봇·자동화 라인용 감속기 교체 전용 장비(크린롤)",
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4">
      <div className="border-l-8 border-sky-700 mb-10 md:mb-16 pl-8 opacity-70">
        ...
        <p className="text-black/50 text-base">대동정밀 제품을 소개합니다.</p>
      </div>
        ...
        {products.map((product, index) => (
          <Link key={index} href={product.link} className="flex flex-col bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md hover:border-sky-400 transition group overflow-hidden">
            <div className="relative w-full h-64">
              <Image src={product.imgSrc} alt={product.title} fill className="object-cover rounded-t-xl" />
            </div>
            <div className="flex flex-col gap-2 p-5">
              <h4 className="font-bold text-slate-800 group-hover:text-sky-700 transition break-keep">{product.title}</h4>
